...
// P0-HOST-001 (2026-06-11): Host vs listener controls. Raise hand handler. End room for host.
// LILCKY STUDIO LIMITED

import { Link, useParams, useNavigate } from "react-router-dom";
import { ChevronLeft, Mic, MicOff, Heart, Hand, MessageCircle, MoreHorizontal, Users, Loader2, PhoneOff } from "lucide-react";
...
export default function RoomLaunchPage() {
  ...
  const handleEndRoom = async () => {
    if (!roomId || !isHost || endingRoom) return;
    setEndingRoom(true);
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Could not end room");
      setEndingRoom(false);
    }
  };
  ...
  const handleRaiseHand = () => {
    ...
    setHandRaised(next);
    if (next) {
      toast.success("Hand raised — the host will invite you to speak");
    } else {
      toast("Hand lowered");
    }
  };
    ...
    return (
      <div className="min-h-screen flex flex-col bg-gradient-to-b from-background via-background to-accent/30">
        ...
        <div className="flex-1 px-6 flex flex-col justify-center text-center">
          ...
          >
            {isHost ? "Start room" : "Join as listener"}
          </button>
          {!isHost && (
      ...
      <header className="sticky top-0 z-30 bg-background/90 backdrop-blur-xl border-b border-border px-3 py-2.5 flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {isHost && (
              <span className="rounded-full bg-neon/15 px-1.5 py-0.5 text-[9px] font-bold uppercase text-neon tracking-wide">
                Host
              </span>
            )}
          </div>
        ...
        {isHost && (
          <div className="flex items-center justify-around px-5 py-3">
            ...
            >
              ...
              <span className="text-[9px]">End room</span>
            </button>
          </div>
        )}

        {/* Main controls — LISTENER view */}
        {!isHost && (
          <div className="flex items-center justify-around px-5 py-3">
            <button
              ...
              className={cn(
                ...
                handRaised ? "text-neon" : ""
              )}
            >
              <div className={cn(
                "h-8 w-8 rounded-full flex items-center justify-center transition-colors",
                handRaised ? "bg-neon text-neon-foreground" : "bg-secondary"
              )}>
                <Hand className="h-4 w-4" />
              </div>
              <span className="text-[9px]">{handRaised ? "Hand up" : "Raise hand"}</span>
            </button>
