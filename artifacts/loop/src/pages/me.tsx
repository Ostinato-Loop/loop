...
// RETENTION-005 (2026-06-10): Notifications row now links to /notifications.
// Previously all rows navigated to /settings, making the bell icon unreachable.
const SETTINGS_ITEMS: { icon: typeof Bell; label: string; sub: string; to: string }[] = [
  { icon: Bell,     label: "Notifications",           sub: "Manage alerts & sounds",        to: "/notifications" },
  { icon: Globe2,   label: "Language & commentary",   sub: "Preferred room language",       to: "/settings" },
  { icon: Shield,   label: "Privacy",                 sub: "Who can see & follow you",      to: "/settings" },
  { icon: Settings, label: "Audio quality",           sub: "Bandwidth & codec preferences", to: "/settings" },
];
    ...
    <AppShell>
      ...
      <div className="-mt-14 px-5">
        ...
        <div className="rounded-2xl border border-border overflow-hidden divide-y divide-border">
          {SETTINGS_ITEMS.map(({ icon: Icon, label, sub, to }) => (
            <button
              key={label}
              type="button"
              onClick={() => navigate(to)}
            ...
            >
              <div className="relative h-9 w-9 rounded-xl bg-secondary flex items-center justify-center shrink-0">
